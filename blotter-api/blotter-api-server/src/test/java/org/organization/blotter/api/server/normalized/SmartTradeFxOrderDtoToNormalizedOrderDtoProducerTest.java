 ...
 */
public class SmartTradeFxOrderDtoToNormalizedOrderDtoProducerTest {

	private SmartTradeFxOrderDtoToNormalizedOrderDtoProducer underTest;
	private ObjectMapper objectMapper;

	@Before
	public void before() {
		objectMapper = new BlotterSharedSerializationConfiguration().objectMapper();
		underTest = new SmartTradeFxOrderDtoToNormalizedOrderDtoProducer(objectMapper);
	}
	...
	public void accept_ok() throws JsonProcessingException {
		...
		// When
		final boolean actual = underTest.accept(ProcessingContext.builder().message(objectMapper.writeValueAsString(dto))
				.source(SourceQueues.SMART_TRADE).timestamp(Instant.now()).build());
